...
/**
 * @title PassportDelegationLogicV5
 ...
 */
library PassportDelegationLogicV5 {
  ...
  function _checkDelegation(
    ...
    if (PassportEntityLogicV5.isEntity(delegator) || self.pendingLinksEntityToPassport[delegator] != address(0)) {
      revert PassportDelegationFromEntity();
    }

    // Check if the delegatee is an entity linked to a passport or has a pending link
    if (PassportEntityLogicV5.isEntity(delegatee) || self.pendingLinksEntityToPassport[delegatee] != address(0)) {
